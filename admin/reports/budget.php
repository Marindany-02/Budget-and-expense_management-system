...
if (!$user_id) {
    die("You must be logged in to view this report.");
}
?>
        ...
        <div id="printable">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    $i = 1;
                    $total = 0;
                    // Fetching data based on user_id and the selected date range
                    $user = $conn->query("SELECT id FROM users WHERE username = '{$_SESSION['username']}'")->fetch_assoc(); // Fetch user row
$user_id = $user['id']; // Get user_id from the row
                    $qry = $conn->query("SELECT r.*, c.category, c.balance FROM `running_balance` r 
                                         INNER JOIN `categories` c ON r.category_id = c.id 
                                         WHERE c.status = 1 AND r.balance_type = 1 
                                         AND r.user_id = '{$user_id}' 
                                         AND DATE(r.date_created) BETWEEN '{$date_start}' AND '{$date_end}' 
